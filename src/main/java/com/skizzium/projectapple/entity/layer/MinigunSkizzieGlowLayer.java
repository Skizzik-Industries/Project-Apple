...
@OnlyIn(Dist.CLIENT)
public class MinigunSkizzieGlowLayer<T extends Entity, M extends EntityModel<T>> extends LayerRenderer<T, M> {
    public MinigunSkizzieGlowLayer(IEntityRenderer<T, M> entity) {
        super(entity);
    }
    ...
    public void render(MatrixStack matrix, IRenderTypeBuffer buffer, int light, T entity, float limbSwing, float limbSwingAmount, float partialTicks, float age, float headYaw, float headPitch) {
        IVertexBuilder builder = buffer.getBuffer(RenderType.eyes(new ResourceLocation("skizzik:textures/entity/skizzie/minigun_skizzie_glow.png")));

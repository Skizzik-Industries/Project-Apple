...
@OnlyIn(Dist.CLIENT)
public class SkizzoGlowLayer<T extends Entity, M extends EntityModel<T>> extends RenderLayer<T, M> {
    public SkizzoGlowLayer(RenderLayerParent<T, M> entity) {
        super(entity);
    }
    ...
    public void render(PoseStack matrix, MultiBufferSource buffer, int light, T entity, float limbSwing, float limbSwingAmount, float partialTicks, float age, float headYaw, float headPitch) {
        VertexConsumer builder = buffer.getBuffer(RenderType.eyes(new ResourceLocation("skizzik:textures/entity/skizzik/skizzo_glow.png")));
